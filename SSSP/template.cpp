...
void compute(int source){
  ...
  while (true){
    ...
    for(int batch_id = 0; batch_id < num_batches; batch_id++){
      if(mode == 0){
        Kernel_s(h_graph_nodes, h_graph_edges, h_graph_weights, h_cost, h_cost_ind, &g_stop, no_of_nodes, edge_list_size, source, batch_size, batch_id, stale_mode);
      }else{
        Kernel_m(h_graph_nodes, h_graph_edges, h_graph_weights, h_cost, h_cost_ind, &g_stop, no_of_nodes, edge_list_size, source, batch_size, batch_id, stale_mode);
      }  
